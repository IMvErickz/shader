...
const signUpSchema = z.object({
    ...
    name: z.string({ message: 'O nome precisa ter no mínimo três caracteres' })
        .min(3, { message: 'O nome precisa ter no mínimo três caracteres' }),
    password: z.string({ message: 'Senha de no mínimo oito caracteres' })
...
export function SingUpForm() {
    ...
    const { control, handleSubmit, watch, formState: { errors, isSubmitting, isValid } } = useForm<SignUpData>({
    ...
    return (
        <form onSubmit={handleSubmit(handleCreateUser)} className="w-[445px] h-max flex flex-col gap-y-4 pt-4">
            <Controller
                control={control}
                name="name"
                render={({ field }) => {
            ...
            />
            <InputPassword placeholder="Confirme sua senha" onChange={event => setPassword(event.target.value)} />
